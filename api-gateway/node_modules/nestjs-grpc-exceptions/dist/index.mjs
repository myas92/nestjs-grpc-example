...
function errorObject(error, code) {
  return {
    message: JSON.stringify({
      ...
      exceptionName: RpcException2.name
    }),
    code
  };
}
...
// lib/exceptions/not-found.exception.ts
var GrpcNotFoundException = class extends RpcException3 {
  constructor(error) {
...
import { RpcException as RpcException4 } from "@nestjs/microservices";
var GrpcAbortedException = class extends RpcException4 {
  constructor(error) {
    super(errorObject(error, status2.ABORTED));
  }
};

// lib/exceptions/unknown.exception.ts
import { status as status3 } from "@grpc/grpc-js";
import { RpcException as RpcException5 } from "@nestjs/microservices";
var GrpcUnknownException = class extends RpcException5 {
  constructor(error) {
    super(errorObject(error, status3.UNKNOWN));
  }
};

// lib/exceptions/internal.exception.ts
import { status as status4 } from "@grpc/grpc-js";
import { RpcException as RpcException6 } from "@nestjs/microservices";
var GrpcInternalException = class extends RpcException6 {
  constructor(error) {
    super(errorObject(error, status4.INTERNAL));
  }
};

// lib/exceptions/unavailable.exception.ts
import { status as status5 } from "@grpc/grpc-js";
import { RpcException as RpcException7 } from "@nestjs/microservices";
var GrpcUnavailableException = class extends RpcException7 {
  constructor(error) {
    super(errorObject(error, status5.UNAVAILABLE));
  }
};

// lib/exceptions/unauthenticated.exception.ts
import { status as status6 } from "@grpc/grpc-js";
import { RpcException as RpcException8 } from "@nestjs/microservices";
var GrpcUnauthenticatedException = class extends RpcException8 {
  constructor(error) {
    super(errorObject(error, status6.UNAUTHENTICATED));
  }
};

// lib/exceptions/permission-denied.exception.ts
import { status as status7 } from "@grpc/grpc-js";
import { RpcException as RpcException9 } from "@nestjs/microservices";
var GrpcPermissionDeniedException = class extends RpcException9 {
  constructor(error) {
    super(errorObject(error, status7.PERMISSION_DENIED));
  }
};

// lib/exceptions/invalid-argument.exception.ts
import { status as status8 } from "@grpc/grpc-js";
import { RpcException as RpcException10 } from "@nestjs/microservices";
var GrpcInvalidArgumentException = class extends RpcException10 {
  constructor(error) {
    super(errorObject(error, status8.INVALID_ARGUMENT));
  }
};

// lib/exceptions/resource-exhausted.exception.ts
import { status as status9 } from "@grpc/grpc-js";
import { RpcException as RpcException11 } from "@nestjs/microservices";
var GrpcResourceExhaustedException = class extends RpcException11 {
  constructor(error) {
    super(errorObject(error, status9.RESOURCE_EXHAUSTED));
  }
};

// lib/exceptions/already-exists.exception.ts
import { status as status10 } from "@grpc/grpc-js";
import { RpcException as RpcException12 } from "@nestjs/microservices";
var GrpcAlreadyExistsException = class extends RpcException12 {
  constructor(error) {
    super(errorObject(error, status10.ALREADY_EXISTS));
  }
};

// lib/exceptions/cancelled.exception.ts
import { status as status11 } from "@grpc/grpc-js";
import { RpcException as RpcException13 } from "@nestjs/microservices";
var GrpcCancelledException = class extends RpcException13 {
  constructor(error) {
    super(errorObject(error, status11.CANCELLED));
  }
};
...
import { RpcException as RpcException14 } from "@nestjs/microservices";
  ...
  intercept(_context, next) {
    return next.handle().pipe(
      (0, import_operators.catchError)((err) => {
        ...
        if (exception.exceptionName !== RpcException14.name)
          return (0, import_rxjs2.throwError)(() => err);
        const statusCode = HTTP_CODE_FROM_GRPC[err.code] || HttpStatus2.INTERNAL_SERVER_ERROR;
        return (0, import_rxjs2.throwError)(
          () => new HttpException(
            {
              message: exception.error,
              statusCode,
              error: HttpStatus2[statusCode]
            },
            statusCode,
            {
              cause: err
...
var GRPC_EXCEPTION_FROM_HTTP = {
  [HttpStatus3.NOT_FOUND]: GrpcNotFoundException,
  [HttpStatus3.FORBIDDEN]: GrpcPermissionDeniedException,
  [HttpStatus3.METHOD_NOT_ALLOWED]: GrpcAbortedException,
  [HttpStatus3.INTERNAL_SERVER_ERROR]: GrpcInternalException,
  [HttpStatus3.TOO_MANY_REQUESTS]: GrpcResourceExhaustedException,
  [HttpStatus3.BAD_GATEWAY]: GrpcUnknownException,
  [HttpStatus3.CONFLICT]: GrpcAlreadyExistsException,
  [HttpStatus3.UNPROCESSABLE_ENTITY]: GrpcInvalidArgumentException,
  [HttpStatus3.UNAUTHORIZED]: GrpcUnauthenticatedException
};
